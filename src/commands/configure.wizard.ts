...
async function promptWebToolsConfig(
	...
	note(
		[
			...
			"It requires a provider API key (Brave, Perplexity/OpenRouter, Grok, or Tavily).",
		...
		const keyInput = guardCancel(
			await text({
				message: hasSearchKey
					? "Brave Search API key (legacy field; leave blank to keep current or use BRAVE_API_KEY)"
					: "Brave Search API key (legacy field; leave blank to use BRAVE_API_KEY)",
		...
		if (key) {
			...
			note(
				[
					...
					"Store a provider key in config or set the provider env var (for example BRAVE_API_KEY or TAVILY_API_KEY).",
					"Docs: https://docs.openclaw.ai/tools/web",
				].join("\n"),
				"Web search",
			);
