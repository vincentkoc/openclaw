...
import { resolveSearchProviderOptions, runSearchSetupFlow } from "./search-setup.js";

const mockedSearchProviders = vi.hoisted(() => {
  const createMockGrokProvider = () => ({
    id: "grok",
    pluginId: "xai",
    label: "Grok",
  ...
  return {
    createMockGrokProvider,
    providers: [createMockGrokProvider()],
  };
});

vi.mock("../plugins/web-search-providers.runtime.js", () => ({
  resolvePluginWebSearchProviders: () => mockedSearchProviders.providers,
}));

describe("runSearchSetupFlow", () => {
  afterEach(() => {
    mockedSearchProviders.providers = [mockedSearchProviders.createMockGrokProvider()];
  });

  it("defaults search providers to setup-visible when onboardingScopes are omitted", () => {
    mockedSearchProviders.providers = [
      {
        ...mockedSearchProviders.createMockGrokProvider(),
        id: "duckduckgo",
        pluginId: "duckduckgo",
    ...
    expect(resolveSearchProviderOptions().map((provider) => provider.id)).toEqual(["duckduckgo"]);
  });
