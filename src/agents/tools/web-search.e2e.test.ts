...
describe("web_search tavily config resolution", () => {
	it("uses config apiKey when provided", () => {
		expect(resolveTavilyApiKey({ apiKey: "tvly-config" })).toBe("tvly-config");
	});

	it("uses TAVILY_API_KEY from env when config key is missing", () => {
		withEnv({ TAVILY_API_KEY: "tvly-env" }, () => {
			expect(resolveTavilyApiKey({})).toBe("tvly-env");
		});
	});
	...
	it("defaults Tavily baseUrl when omitted", () => {
		expect(resolveTavilyBaseUrl({})).toBe("https://api.tavily.com");
		expect(resolveTavilyBaseUrl(undefined)).toBe("https://api.tavily.com");
	});

	it("uses configured Tavily baseUrl when provided", () => {
		expect(resolveTavilyBaseUrl({ baseUrl: "https://tavily.proxy.local" })).toBe(
			"https://tavily.proxy.local",
		);
	});
});
